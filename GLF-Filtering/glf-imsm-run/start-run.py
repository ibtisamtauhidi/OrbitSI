...
FILTERS = ["GQL", "TSO", "DPiso", "NLF", "GLFP"]
ORDERS = ["GQL", "TSO", "DPiso", "VF2PP", "RI"]
ENGINE = ["GQL", "EXPLORE", "EXPLORE", "EXPLORE", "LFTJ"]
...
for i in range(len(ORDERS)):
	for f in FILES:	
		...
		mdict["Filter"] = FILTERS[i]
		mdict["Order"] = ORDERS[i]
		mdict["Explore"] = ENGINE[i]
		arr.append(mdict)
...
#SBATCH --output=./output/"""+DATASET+"""/R-%x.%j.out
#SBATCH --mem=32G
...
"""
	...
	with open("./scripts/start-"+str(i+1)+".sh", "w") as text_file:
