...
void GenerateQueryPlan::generateGLF_HOF_QueryPlan(const Graph *data_graph, const Graph *query_graph, ui *&order, ui *&pivot, std::vector<std::vector<ui>> query_orbits) {
    ui query_vertices_num = query_graph->getVerticesCount();
    order = new ui[query_vertices_num];
    pivot = new ui[query_vertices_num];

	std::vector<ui> orbit_ss(query_vertices_num, 0);
    for (ui i = 1; i < query_vertices_num; ++i) {
		for (ui j = 1; j < 15; ++j) {
			orbit_ss[i] += query_orbits[i][j]*query_orbits[i][j];
		}	
    }	
    
    std::vector<ui> indices(query_vertices_num);
    std::iota(indices.begin(), indices.end(), 0);

    std::sort(indices.begin(), indices.end(), [&](ui a, ui b) {
        return orbit_ss[a] < orbit_ss[b];
    });
...
void GenerateQueryPlan::generateGLF_DFS_QueryPlan(const Graph *data_graph, const Graph *query_graph, ui *&order, ui *&pivot, std::vector<std::vector<ui>> query_orbits) {
    ui query_vertices_num = query_graph->getVerticesCount();
    order = new ui[query_vertices_num];
    pivot = new ui[query_vertices_num];
    ...
    for (ui i = 1; i < query_vertices_num; ++i) {
		...
		for (ui j = 1; j < 16; ++j) {
			orbit_ss[i] += query_orbits[i][j]*query_orbits[i][j];
			// std::cout << query_orbits[i][j];
			// if(j != 15) std::cout << ", ";
			// else  std::cout << " | ";
		}
		// std::cout << orbit_ss[i] << std::endl;
    }

    std::vector<bool> visited(query_vertices_num, false);

    // Select the vertex with the maximum orbit_ss as the start vertex.
    order[0] = 0;
    for (ui i = 1; i < query_vertices_num; ++i) {
        if (orbit_ss[i] > orbit_ss[order[0]]) {
            order[0] = i;
        }
    }
        ...
        for (ui u = 0; u < query_vertices_num; ++u) {
            if (!visited[u]) {
                ...
                if (cur_bn > max_bn) {
                    ...
                    if(tie_vertices.size() == 0) {
		                tie_vertices.clear();
		                tie_vertices.push_back(u);                    
		             } else if(orbit_ss[u] > orbit_ss[tie_vertices[0]]) {
		                tie_vertices.clear();
		                tie_vertices.push_back(u);                    
		             }
                }
