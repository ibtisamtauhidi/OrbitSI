...
bool
FilterVertices::GLFFilter(const Graph *data_graph, const Graph *query_graph, ui **&candidates, ui *&candidates_count, std::vector<std::vector<ui>> data_orbits, std::vector<std::vector<ui>> query_orbits) {
    ui** candidates_temp = NULL;
    ui* candidates_count_temp = NULL;
    allocateBuffer(data_graph, query_graph, candidates, candidates_count);
    allocateBuffer(data_graph, query_graph, candidates_temp, candidates_count_temp);

    for (ui i = 0; i < query_graph->getVerticesCount(); ++i) {
        VertexID query_vertex = i;
        computeCandidateWithNLF(data_graph, query_graph, query_vertex, candidates_count_temp[query_vertex], candidates_temp[query_vertex]);
        if (candidates_count_temp[query_vertex] == 0) {
            return false;
        }
    }
    ...
    for (ui i = 0; i < query_graph->getVerticesCount(); ++i) {
		int count = 0;
    	for (ui j = 0; j < candidates_count_temp[i]; ++j) {
			bool addCandidate = true;
			for (int k = 0; k < 15; ++k) {
				if (data_orbits[candidates_temp[i][j]][k] < query_orbits[i][k]) addCandidate = false;
			}
			if (addCandidate) candidates[i][count++] = candidates_temp[i][j];
		}
...
bool
FilterVertices::GLFPFilter(const Graph *data_graph, const Graph *query_graph, ui **&candidates, ui *&candidates_count,
                            ui *&order, TreeNode *&tree, std::vector<std::vector<ui>> data_orbit, std::vector<std::vector<ui>> query_orbit) {

    if (!GLFFilter(data_graph, query_graph, candidates, candidates_count, data_orbit, query_orbit)) return false;
