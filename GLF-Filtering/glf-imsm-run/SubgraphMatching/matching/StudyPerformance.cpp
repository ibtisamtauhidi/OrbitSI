...
#define NANOSECTOMILLISEC(elapsed_time) ((elapsed_time)/(double)1000000000)
...
std::vector<std::vector<ui>> readOrbits(const std::string& filename) {
    std::vector<std::vector<ui>> data;
    ...
    while (std::getline(file, line)) {
        ...
        while (std::getline(iss, token, ',')) {
            try {
                ui value = std::stoul(token);
                row.push_back(value);
            } catch (const std::exception& e) {
                std::cerr << "Error: Invalid integer value in CSV: " << token << std::endl;
            }
        }
        data.push_back(row);
    }
...
int main(int argc, char** argv) {
    ...
    std::vector<std::vector<ui>> query_orbits = readOrbits(getOrbitPath(input_query_graph_file));
    std::vector<std::vector<ui>> data_orbits = readOrbits(getOrbitPath(input_data_graph_file));
    ...
    } else if (input_filter_type == "GLFP") {
        FilterVertices::GLFPFilter(data_graph, query_graph, candidates, candidates_count, dpiso_order, dpiso_tree, data_orbits, query_orbits);
    ...
    // Added by Syed - Print Candidate Set
    //std::cout << "--------------------------------------------------------------------" << std::endl;
    //std::cout << "SYED-CS-BEGIN" << std::endl;
    //for (int i = 0; i < query_graph->getVerticesCount(); ++i) {
    //    std::cout << i << ":" << candidates_count[i];
    //	for (int j = 0; j < candidates_count[i]; ++j) {
	//	    std::cout << candidates[i][j] << " ";
	//	}
		//std::cout << std::endl;	
    //}
    //std::cout << "SYED-CS-END" << std::endl;
    //std::cout << "--------------------------------------------------------------------" << std::endl;
    ...
    else if (input_order_type == "GLFHOF") {
        GenerateQueryPlan::generateGLF_HOF_QueryPlan(data_graph, query_graph, matching_order, pivots, query_orbits);
    }
    else if (input_order_type == "GLFDFS") {
        GenerateQueryPlan::generateGLF_DFS_QueryPlan(data_graph, query_graph, matching_order, pivots, query_orbits);
    }
...
//                                                           call_count);
    ...
    printf("Load graphs time (seconds): %.8lf\n", NANOSECTOMILLISEC(load_graphs_time_in_ns));
    printf("Filter vertices time (seconds): %.8lf\n", NANOSECTOMILLISEC(filter_vertices_time_in_ns));
    printf("Build table time (seconds): %.8lf\n", NANOSECTOMILLISEC(build_table_time_in_ns));
    printf("Generate query plan time (seconds): %.8lf\n", NANOSECTOMILLISEC(generate_query_plan_time_in_ns));
    printf("Enumerate time (seconds): %.8lf\n", NANOSECTOMILLISEC(enumeration_time_in_ns));
    printf("Preprocessing time (seconds): %.8lf\n", NANOSECTOMILLISEC(preprocessing_time_in_ns));
    printf("Total time (seconds): %.8lf\n", NANOSECTOMILLISEC(total_time_in_ns));
    printf("Memory cost (MB): %.8lf\n", BYTESTOMB(memory_cost_in_bytes));
